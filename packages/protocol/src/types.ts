...
// `seg` increments whenever the measured quantity changes.
...
// it and force a new segment (flags C bit3).
export function quantityKey(r: Reading): string {
  return `${r.function}|${r.acdc}`;
}

// One channel in a recording. A recording spans N channels — meter
...
// Index = frame[3] & 0x7F (docs/protocols/uni-t.md). Codes 0–21 were verified on our UT60BT. Codes 22–31
// are ported from the open-source `Bluetooth-DMM-For-Windows` project's `DecoderUni_T.cs` `functionStrings` for other UT-series
...
// and we fall back to [0] for any range (docs/protocols/uni-t.md). '' = no unit
