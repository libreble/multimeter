// owon-old driver — the legacy OWON B35T(+) text protocol (the open-source `Bluetooth-DMM-For-Windows` project's
...
// (`b35tDecodeOld`), then validated byte-exact against the vendor OWON app's decoder via the fake-ble-meter emulator oracle (a hardware-free bench
...
//   * byte 6 : decimal-point position, an ASCII DIGIT (NOT a bitmask) — the vendor app
//              switches on the char: '1'(0x31)→3 decimals,
...
export function decodeOwonOld(bytes: Uint8Array, ts = 0): Reading {
  ...
  // Decimal-point position: byte6 is an ASCII digit (the vendor app switches on the char),
  // NOT a bitmask. '1'(0x31)→3 decimals, '2'(0x32)→2, '4'(0x34)→1, else→0.
  ...
  // prefix lives in byte 8 bit 1, an annunciator independent of the
