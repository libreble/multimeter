...
// rebadges). DevType 2 (`isBDM == 2`) in the open-source `Bluetooth-DMM-For-Windows` project's GATT table, GATT service 0xFFF0.
//
...
// byte-exact against the vendor OWON app's decoder via the fake-ble-meter emulator oracle (a hardware-free bench test): the scale/function
// nibbles, the SI-prefix + unit tables, and the LSB-first flag order all match
...
export function decodeOwonPlus(bytes: Uint8Array, ts = 0): Reading {
  ...
  // RMR(7), PMIN(8), PMAX(9), UL(10), LPF0(11), LPF1(12), VBAR(13)). Flag n lives at bit n. (The earlier C#-port reading indexed an MSB-first padded string and put HOLD
