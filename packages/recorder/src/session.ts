// RecorderSession — the framework-agnostic, multi-channel recording engine . Turns N fed Reading streams (meter + derived channels) into what the
// multi-series chart / per-channel stats / shared recording need, from one history buffer per
// channel: a bounded current-segment buffer (chart + live stats, always while
